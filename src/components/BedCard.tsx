...
export default function BedCard({ id, type, label, assignedPeople, onUnassign, cost, onDelete }: BedCardProps) {
      ...
      <div className="flex items-center gap-1.5 justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <div className="min-w-0">
            ...
            {cost !== undefined && (
              <div className="text-xs text-gray-400">${cost.toLocaleString('en-US', { maximumFractionDigits: 0 })}</div>
            )}
          </div>
        </div>
        {onDelete && (
          <button onClick={onDelete} className="text-gray-600 hover:text-red-400 text-xs leading-none flex-shrink-0">×</button>
        )}
      </div>
